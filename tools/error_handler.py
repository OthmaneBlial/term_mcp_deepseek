...
class CommandExecutionError(MCPError):
    ...
    def __init__(self, command: str, error: str):
        super().__init__(
            f"Command execution failed: {command}", 500, {"command": command, "error": error}
        )
...
class ErrorHandler:
    ...
    def handle_flask_error(error: Exception) -> tuple:
        ...
        if hasattr(error, "code"):
            if error.code == 400:
                ...
                return jsonify(
                    {"error": "Method Not Allowed", "message": "HTTP method not supported"}
                ), 405
    ...
    def handle_jsonrpc_error(error: Exception) -> dict[str, Any]:
        ...
        return {
            "jsonrpc": "2.0",
            "error": {"code": -32603, "message": "Internal error", "data": str(error)},
            "id": None,
        }
    ...
    @staticmethod
    def handle_auth_error(message: str, user_id: str | None = None) -> AuthenticationError:
    ...
    def _format_jsonrpc_error(error: MCPError) -> dict[str, Any]:
        ...
        jsonrpc_error = {"code": error.code, "message": error.message}

        if error.details:
            jsonrpc_error["data"] = error.details

        return {"jsonrpc": "2.0", "error": jsonrpc_error, "id": None}

    @staticmethod
    def log_operation(operation: str, success: bool, details: dict[str, Any] | None = None):
